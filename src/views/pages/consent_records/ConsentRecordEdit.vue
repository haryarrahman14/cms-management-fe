  ...
  <v-container>
    <BaseBreadcrumb
      :items="[
        ...
        {
          text: `${recordData?.cif || ''} - ${recordData?.userChannel || ''}`,
          to: `/consent-records/detail/${recordId}`,
          active: false,
        },
        { text: 'EDIT USER CONSENT', active: true },
      ]"
    />
    ...
    <v-card class="mt-4">
      <v-card-title>
        <h1 class="text-h5 font-weight-bold mb-4 pt-3">Edit Consent User</h1>
      </v-card-title>
      <v-card-text>
        <v-form ref="formRef" v-model="formValid">
          <v-row dense>
            ...
            <v-col cols="12">
              <BaseInput v-model="form.cif" label="CIF" readonly disabled />
            </v-col>
            <v-col cols="12">
              <BaseInput v-model="form.userChannel" label="Nama Nasabah" readonly disabled />
            </v-col>
            <v-col cols="12">
              <BaseInputSelect
...
const { data: recordData, error: recordError } =
  ConsentSubmissionService.useGetConsentSubmissionDetail(recordId, {
...
const {
  mutate: updateConsent,
  isPending: isSubmitting,
  error: updateError,
} = ConsentSubmissionService.useUpdateConsentSubmission(recordId, {
  onSuccess: () => {
    snackbar.open('Data consent berhasil diupdate!')
...
const error = computed(() => recordError.value || updateError.value)
watch(
  () => recordData.value,
  (newRecord) => {
    if (newRecord) {
      form.submitDate = newRecord.submitDate ? Format.dateDefault(newRecord.submitDate) : null
      form.cif = newRecord.cif
      form.userChannel = newRecord.userChannel
      form.consentFormId = newRecord.consentFormId
      form.version = newRecord.version?.toString()
      form.channel = newRecord.channel
...
watch(
  [() => templateDetail.value, () => recordData.value],
    ...
    if (newTemplate && newRecord) {
      ...
      form.decisions = newTemplate.sections.map((section) => {
        ...
        return {
          ...
          accepted: existingDecision?.accepted ?? null,
        }
      })
    }
  },
